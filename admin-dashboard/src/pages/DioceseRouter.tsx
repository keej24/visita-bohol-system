/**
 * =============================================================================
 * DIOCESE ROUTER - Smart Dashboard Routing Based on User Role
 * =============================================================================
 *
 * PURPOSE:
 * This component is the "traffic controller" after login. It examines the
 * user's role and diocese, then routes them to their appropriate dashboard.
 * Think of it as an automatic switchboard that connects you to the right place.
 *
 * ROUTING LOGIC:
 * ┌─────────────────────────┬─────────────────────────────────────────────────┐
 * │ User Role               │ Destination                                     │
 * ├─────────────────────────┼─────────────────────────────────────────────────┤
 * │ parish_secretary        │ LazyParishDashboard                             │
 * │ museum_researcher       │ LazyMuseumResearcherDashboard                   │
 * │ chancery_office         │ Based on diocese:                               │
 * │   - tagbilaran         │   → LazyTagbilaranDashboard                     │
 * │   - talibon            │   → LazyTalibonDashboard                        │
 * │   - (none)             │   → DioceseSelection (choose a diocese)         │
 * └─────────────────────────┴─────────────────────────────────────────────────┘
 *
 * WHY THIS COMPONENT EXISTS:
 * Instead of having complex routing logic in App.tsx, this component
 * handles the "which dashboard should I show?" decision in one place.
 * It makes App.tsx simpler and keeps routing logic centralized.
 *
 * LOADING STATES:
 * 1. Auth loading: Firebase checking if user is logged in
 * 2. Profile creating: First-time login for known accounts (auto-creating profile)
 * 3. Profile timeout: User logged in but profile won't load (shows error)
 *
 * ERROR HANDLING:
 * - No user → Redirect to /login
 * - User but no profile after 5 seconds → Show error with refresh button
 * - Profile creating → Show "Setting up your account..." message
 *
 * DIOCESE SELECTION:
 * If a chancery_office user doesn't have a diocese assigned, they see
 * DioceseSelection component which shows cards for both dioceses.
 * This is rare - most users have diocese pre-assigned.
 *
 * LAZY LOADING:
 * All dashboards use React.lazy() for code splitting.
 * Each dashboard only loads when needed, keeping initial bundle small.
 * Suspense shows a spinner while dashboard code downloads.
 *
 * RELATED FILES:
 * - App.tsx: Defines "/" route pointing here
 * - contexts/AuthContext.tsx: Provides userProfile, loading states
 * - components/LazyComponents.tsx: Lazy-loaded dashboard components
 * - pages/TagbilaranDashboard.tsx, etc.: Actual dashboard pages
 */
