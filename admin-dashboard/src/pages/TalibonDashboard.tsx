// Diocese-specific dashboard for Talibon
...
import { Calendar, Clock, FileText, AlertCircle, MapPin, Plus } from "lucide-react";
...
const TalibonDashboard = () => {
  const { userProfile } = useAuth();
  const { data: churches, isLoading } = useQuery<Church[]>({
    queryKey: ["churches", "talibon"],
    queryFn: () => getChurchesByDiocese("talibon"),
  });
  const pendingCount = (churches ?? []).filter(c => ["pending", "needs_revision", "heritage_review"].includes(c.status)).length;
  const heritageCount = (churches ?? []).filter(c => c.classification === "ICP" || c.classification === "NCT").length;
    ...
    <Layout>
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 justify-between">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-bold text-primary mb-1">
                Diocese of Talibon - Chancery Office
              </h1>
              <p className="text-muted-foreground">
                Managing churches and parishes in the Diocese of Talibon
              </p>
            ...
            <CreateParishAccountModal diocese="talibon" trigger={
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <ChanceryReviewList diocese="talibon" />
        </div>
